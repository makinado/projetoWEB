    ...
    <v-dialog v-model="modalStore.empresas.metas.visible" lazy persistent max-width="900px">
      <v-card>
                          ...
                          <v-layout wrap>
                            ...
                            <v-flex xs12 md1>
                              <v-btn
                                class="v-btn-common"
                                color=""
                                @click="!showTabela ? geraTabela() : geraTabela()"
                                :disabled="!flagData ? true : false"
                              >
                                <v-icon v-if="showTabela">fa fa-lg fa-refresh</v-icon>
                                <template v-else>Gerar</template>
                              </v-btn>
                            </v-flex>
                            <v-flex xs12>
                              <v-data-table
                          ...
                          <v-layout justify-end>
                            <v-tooltip bottom>
                              <v-btn
                                ...
                                :disabled="!showTabela"
                              >
                                <template>Gerar</template>
                              </v-btn>
                              <span>Gera as metas dos vendedores</span>
                            </v-tooltip>
                          </v-layout>
                          <v-layout wrap>
                            <v-flex xs12>
        ...
        <v-card-actions>
          ...
          >Fechar</v-btn>
          <v-btn v-if="tabIndex === 'tab-1'" color="blue darken-1" flat @click="save()">Salvar</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </v-layout>
...
export default {
  ...
  data() {
    return {
      ...
      metaAnual: [
        { mes: "Janeiro", valor: "", percentual: "", expanded: false },
        { mes: "Fevereiro", valor: "", percentual: "", expanded: false },
        { mes: "Março", valor: "", percentual: "", expanded: false },
        { mes: "Abril", valor: "", percentual: "", expanded: false },
        { mes: "Maio", valor: "", percentual: "", expanded: false },
        { mes: "Junho", valor: "", percentual: "", expanded: false },
        { mes: "Julho", valor: "", percentual: "", expanded: false },
        { mes: "Agosto", valor: "", percentual: "", expanded: false },
        { mes: "Setembro", valor: "", percentual: "", expanded: false },
        { mes: "Outubro", valor: "", percentual: "", expanded: false },
        { mes: "Novembro", valor: "", percentual: "", expanded: false },
        { mes: "Dezembro", valor: "", percentual: "", expanded: false }
      ],
      metaVend: [],
      categoriaErrors: [],
      tipoErrors: [],
  ...
  methods: {
    ...
    atualizaVend(metaAtt, calcTotal) {
      ...
      if (calcTotal) {
        ...
        const metas = this.metaAnual.filter(meta => {
          return meta.valor !== "-";
        });

        const meses = metas.length;

        let soma = 0;
        this.metaVend.map(meta => {
          soma += parseFloat(parseNumber(meta.total, ","));
        });

        this.metaVend = this.metaVend.filter(meta => {
          if (meta.id === id) {
            const result = parseFloat(parseNumber(meta.total, ",")) / meses;

            meta.janeiro = result;
            meta.fevereiro = result;
            meta.marco = result;
            meta.abril = result;
            meta.maio = result;
            meta.junho = result;
            meta.julho = result;
            meta.agosto = result;
            meta.setembro = result;
            meta.outubro = result;
            meta.novembro = result;
            meta.dezembro = result;
    ...
    loadTela(meta) {
      const urlTela = `${urlBD}/empresaMetas/TelaMetas`;
      if (!meta) {
    ...
    loadEmpresas() {
      ...
      axios.get(url).then(res => {
        ...
        this.empresaStore.empresas.map(empresa => {
          empresa.value = empresa.id;
          empresa.text = empresa.nome;
        });
    ...
    saveVend() {
      ...
      if (!this.metaVendAtt || this.metaVendAtt.length === 0)
        showError("Selecione os vendedores que deseja gerar a meta!");
    },
    Errors(e) {
      e.response.data.includes("receita")
        ? (this.categoriaErrors = e.response.data)
        : (this.categoriaErrors = []);
      e.response.data.includes("anual")
        ? (this.tipoErrors = e.response.data)
        : (this.tipoErrors = []);
      e.response.data.includes("data")
        ? [(this.dataErrors = e.response.data), (this.tabIndex = "tab-1")]
        : (this.dataErrors = []);
      e.response.data.includes("valor")
        ? [(this.valorErrors = e.response.data), (this.tabIndex = "tab-1")]
        : (this.valorErrors = []);
    }
  }
};
