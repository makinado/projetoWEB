module.exports = app => {
    ...
    const save = async (req, res) => {
        ...
        try {
            ...
            const comissaoDB = await req.knex('usuario_comissao').select('id').where({ tipo: comissao.tipo, id_usuario: comissao.id_usuario, ordem: comissao.ordem, ativa: true }).first()
            if (!comissao.id) {
        ...
        const desativarComissao = await req.knex('usuario_comissao').select('id_usuario', 'tipo').where({ id_usuario: comissao.id_usuario, ativa: true }).whereNot({ tipo: comissao.tipo }).first()
        if (desativarComissao) {
            await req.knex('usuario_comissao').update({ ativa: false }).where({ id_usuario: desativarComissao.id_usuario, tipo: desativarComissao.tipo, ativa: true })
        }
    ...
    const remove = async (req, res) => {
        try {
            const exclusao = await req.knex('usuario_comissao')
